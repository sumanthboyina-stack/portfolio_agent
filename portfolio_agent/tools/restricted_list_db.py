"""
Restricted-list database — compliance blocklist + pipeline phase-skip list.

Tables:
  restricted_list — tickers blocked from new recommendations (ticker, reason, added_at)
  pipeline_skip    — tickers silently skipped in specific pipeline phases
                     (ticker, phases (JSON list), reason, added_at)
...
from portfolio_agent.tools.db import db_conn, from_json_list, to_json_list
...
def add_restricted(ticker: str, reason: str | None) -> None:
    ...
    with _db() as conn:
        conn.execute(
            "INSERT INTO restricted_list (ticker, reason, added_at) VALUES (?, ?, ?) "
            "ON CONFLICT(ticker) DO UPDATE SET reason = excluded.reason",
            (ticker.upper(), reason, now),
        )
        conn.commit()
