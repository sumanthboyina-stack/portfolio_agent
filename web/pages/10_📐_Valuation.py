...
valuation = None if force_recompute else _cached_stored_valuation(ticker_input)
compute_error = None
if valuation is None:
    valuation, compute_error = _cached_compute_valuation(ticker_input)

if compute_error:
    st.error(compute_error, icon="⚠️")
    st.stop()
if valuation is None:
...
# ── Headline: Intrinsic Value / Current Price / Margin of Safety / Label ──────
...
with hcol3:
    ...
    st.markdown(stat_card_html(f"{mos:+.1f}%" if mos is not None else "—", "Margin of Safety", "🛡️", mos_color), unsafe_allow_html=True)
with hcol4:
    st.markdown(
...
for col, (key, label_txt, color, bg) in zip(scen_cols, _SCENARIO_DEFS):
    with col:
        ...
        card(
            ...
            f'<div style="margin-top:6px">{badge_html(f"{prob*100:.0f}% probability", color, bg)}</div>'
            f'</div>',
...
with st.expander("📋 Assumptions used (base case)"):
    ...
    if base_assumptions:
        ...
        with acol1:
            st.markdown(
                f"- Trend revenue growth: **{base_assumptions.get('trend_revenue_growth', 0)*100:.1f}%**\n"
                f"- FCF margin: **{base_assumptions.get('fcf_margin', 0)*100:.1f}%**\n"
                f"- Terminal growth: **{base_assumptions.get('terminal_growth', 0)*100:.1f}%**\n"
                f"- Projection years: **{base_assumptions.get('projection_years', 5)}**"
            )
        with acol2:
            st.markdown(
                f"- WACC: **{base_assumptions.get('wacc', 0)*100:.1f}%** "
                f"(cost of equity {base_assumptions.get('cost_of_equity', 0)*100:.1f}%, "
                f"cost of debt {base_assumptions.get('cost_of_debt_after_tax', 0)*100:.1f}% *approximate*)\n"
                f"- Risk-free rate: **{base_assumptions.get('risk_free_rate', 0)*100:.1f}%**\n"
                f"- Beta used: **{base_assumptions.get('beta_used', 'n/a')}**\n"
                f"- Tax rate: **{base_assumptions.get('tax_rate', 0)*100:.0f}%**"
            )
        st.caption(
...
if rel.get("ticker_multiples"):
    ...
    rows_html = "".join(
        f'<tr><td style="padding:8px 10px;font-weight:600;color:#0F172A">{label}</td>'
    ...
    st.markdown(
        ...
        f'<th style="text-align:right;padding:8px 10px;font-size:0.72rem;color:#94A3B8">{ticker_input}</th>'
    ...
    if hist:
        st.caption(
            f"Approximate 52-week P/E range: {hist['low']:.1f}x – {hist['high']:.1f}x "
